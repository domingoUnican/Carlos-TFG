...
{
    if (!ctx || !sequence || !bound_bit_sequence) {
        return false;
    }
...
static bool build_bound_bit_sequence_excluding_batch(const DFSContext *ctx,
                                                     int batch_start_coset,
                                                     int batch_len,
                                                     int *bound_bit_sequence) {
    if (!ctx || !ctx->cl || !bound_bit_sequence || batch_start_coset < 0 || batch_len < 0) {
        return false;
    }

    memset(bound_bit_sequence, 1, (size_t)ctx->N * sizeof(int));

    int batch_end_coset = batch_start_coset + batch_len;
    if (batch_end_coset > ctx->num_cosets) {
        batch_end_coset = ctx->num_cosets;
    }

    for (int cidx = batch_start_coset; cidx < batch_end_coset; cidx++) {
        Coset *c = &ctx->cl->data[cidx];
        for (int t = 0; t < c->len; t++) {
            int elem = c->data[t];
            if (elem >= 0 && elem < ctx->N) {
                bound_bit_sequence[elem] = 0;
            }
        }
    }
...
bool is_compression_of_candidates(const DFSContext *ctx, int flag)
{
    int dimension_candidate_pairs = (flag == 1) ? ctx->dimension_candidate_pairs1 : ctx->dimension_candidate_pairs2;
    size_t num_candidate_pairs = (flag == 1) ? ctx->num_candidate_pairs1 : ctx->num_candidate_pairs2;
    int **pairs_temp = (flag == 1) ? ctx->candidate_pairs1 : ctx->candidate_pairs2;
    bool **is_less_cand = (flag == 1) ? ctx->is_less_candidates_1 : ctx->is_less_candidates_2;
    const int *compression = (flag == 1) ? ctx->compresion_1 : ctx->compresion_2;
    const int *compression_bound = (flag == 1) ? ctx->bound_compression_1 : ctx->bound_compression_2;
    int depth = ctx->coset_idx;
    bool can_use_prefilter = (is_less_cand && depth >= 0);
    bool exact_by_bounds = true;
    for (int d = 0; d < dimension_candidate_pairs; d++) {
        if (compression_bound[d] != 0) {
            exact_by_bounds = false;
            break;
        }
    }

    for (size_t j = 0; j < num_candidate_pairs; j++) {
        if (can_use_prefilter && !is_less_cand[depth][j]) {
            continue;
        }
        if (exact_by_bounds && can_use_prefilter) {
            return true;
        }
        bool matches = true;
        for (int d = 0; d < dimension_candidate_pairs; d++) {
            if (can_use_prefilter && compression_bound[d] == 0) {
                continue;
            }
            int v = pairs_temp[j][d];
            if (v != compression[d]) {
                matches = false;
                break;
            }
...
bool is_valid_combination(const DFSContext *ctx) {
    ...
    //free(vector_bits);
    int temp_psd = -1;
    bool result = (is_compression_of_candidates(ctx,0) && is_compression_of_candidates(ctx,1));
    if ( result )
    {
        for (int j = 1; (j < ctx->spectrum_size) && result; j++) {
            double real_part = creal(ctx->current_dft[j]);
            double imag_part = cimag(ctx->current_dft[j]);
            temp_psd = (rint)(real_part * real_part + imag_part * imag_part);
            ctx->current_psd[j] = temp_psd;
            result = result && (temp_psd <= ctx->threshold);
        }
...
static double **compute_depth_exploration_bounds(DFSContext *ctx) {
    ...
    int *combo_arr = malloc((size_t)ctx->cl->len * sizeof(int));
    int *bound_bit_sequence = malloc((size_t)N * sizeof(int));
    if (!combo_arr || !bound_bit_sequence) {
        free(combo_arr);
        free(bound_bit_sequence);
        for (int i = 0; i <= num_cosets; i++) {
            free(bounds[i]);
        }
        free(bounds);
    ...
    // bounds[num_cosets] must remain zero: there are no remaining cosets to add.
    if (num_cosets <= 1) {
        return bounds;
    }

    int batch_size = DEPTH;
    int total_i = num_cosets - 1;
    int num_batches = (total_i + batch_size - 1) / batch_size;

    for (int batch_idx = 0; batch_idx < num_batches; batch_idx++) {
        int batch_first_i = batch_idx * batch_size + 1;
        int batch_last_i = (batch_idx + 1) * batch_size;
        if (batch_last_i > total_i) {
            batch_last_i = total_i;
        }

        int batch_start_coset = batch_first_i - 1;
        int batch_len = batch_last_i - batch_first_i + 1;
        int num_combinations = 1 << batch_len;

        if (!build_bound_bit_sequence_excluding_batch(ctx, batch_start_coset, batch_len, bound_bit_sequence)) {
            free(combo_arr);
            free(bound_bit_sequence);
            for (int i = 0; i <= num_cosets; i++) {
                free(bounds[i]);
        ...
        for (int combo = 0; combo < num_combinations; combo++) {
            ...
            for (int k = 0; k < batch_len; k++) {
                if ((combo >> k) & 1) {
                    combo_arr[pos++] = batch_start_coset + k;
                }
            }
            combo_arr[pos] = -1;
            ...
            bool passes = is_less_than_candidates_for_vectors(ctx, seq, bound_bit_sequence, 1) &&
                          is_less_than_candidates_for_vectors(ctx, seq, bound_bit_sequence, 0);
            ...
            if (passes) {
                for (int j = 0; j < ctx->spectrum_size; j++) {
                    ...
                    for (int k = 0; k < batch_len; k++) {
                        if ((combo >> k) & 1) {
                            val += ctx->dft_matrix[batch_start_coset + k][j];
                        }
                    }
                    double abs_val = cabs(val);
                    if (abs_val > bounds[batch_first_i][j]) {
                        bounds[batch_first_i][j] = abs_val;
                    }
                }
            }
        ...
        for (int i = batch_first_i + 1; i <= batch_last_i; i++) {
            memcpy(bounds[i], bounds[batch_first_i], (size_t)ctx->spectrum_size * sizeof(double));
        }
    }

    for (int batch_idx = num_batches - 1; batch_idx >= 0; batch_idx--) {
        int batch_first_i = batch_idx * batch_size + 1;
        int batch_last_i = (batch_idx + 1) * batch_size;
        if (batch_last_i > total_i) {
            batch_last_i = total_i;
        }
        int next_i = batch_last_i + 1;

        for (int i = batch_first_i; i <= batch_last_i; i++) {
            for (int j = 0; j < ctx->spectrum_size; j++) {
                bounds[i][j] += bounds[next_i][j];
            }
        }
    }
