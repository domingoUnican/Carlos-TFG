...
void find_matches_files(size_t N, size_t row_ints, char* path_psd, char* path_cte, char* path_lp, char* path_comb) {
    FILE *f1 = fopen(path_psd, "r");
    FILE *f2 = fopen(path_cte, "r");
    ...
    size_t line_capacity = (row_ints * 16u) + 64u;
    if (line_capacity < 256u) {
        line_capacity = 256u;
    }
    ...
    char **all_lines2 = malloc(capacity * sizeof(char *));
    char *line2 = malloc(line_capacity);
    ...
    while (fgets(line2, (int)line_capacity, f2) != NULL) {
        ...
        if (total_lines2 >= capacity) {
            capacity *= 2;
            char **tmp = realloc(all_lines2, capacity * sizeof(char *));
            if (!tmp) {
                for (size_t k = 0; k < total_lines2; k++) {
                    free(all_lines2[k]);
                }
                free(all_lines2);
                free(line2);
        ...
        all_lines2[total_lines2] = malloc(len + 1u);
        if (!all_lines2[total_lines2]) {
            for (size_t k = 0; k < total_lines2; k++) {
                free(all_lines2[k]);
            }
            free(all_lines2);
        ...
        memcpy(all_lines2[total_lines2], line2, len + 1u);
        total_lines2++;
    }
    free(line2);
    ...
    char *line1_data = malloc(line_capacity);
    if (!line1_data) {
        for (size_t k = 0; k < total_lines2; k++) {
            free(all_lines2[k]);
        }
    ...
    // 2. Leer F1 línea a línea y comparar contra todo F2 en memoria
    rewind(f1);
    while (!LP_found && fgets(line1_data, (int)line_capacity, f1) != NULL) {
        size_t len1 = strlen(line1_data);
        while (len1 > 0 && (line1_data[len1 - 1] == '\n' || line1_data[len1 - 1] == '\r')) {
            line1_data[--len1] = '\0';
        }
        ...
        while (j < total_lines2 && !LP_found) {
            ...
            if (line1_num < j) {
                // Comparamos la línea actual de f1 con la línea j en RAM de f2
                if (strcmp(line1_data, all_lines2[j]) == 0) {
...
bool check_bound(DFSContext *ctx) {
    ...
    double root = sqrt((double)ctx->threshold);
    for (int j = 1; j < ctx->spectrum_size && result; j++) {
        double real_part = creal(ctx->current_dft[j]);
        double imag_part = cimag(ctx->current_dft[j]);
        double current_abs = real_part * real_part + imag_part * imag_part;
        ctx->current_psd[j] = (int)rint(current_abs);
        double lower_bound_psd = sqrt(current_abs) - ctx->current_bound[j];
        result = result && (lower_bound_psd <= root);
    }
    return result;
}

bool is_valid_combination(const DFSContext *ctx) {
    // Precondicion: current_psd contiene la PSD del estado actual (actualizada en check_bound).

    bool result = false;
    if (is_compression_of_candidates(ctx,ctx->current_sequence,0) && 
        is_compression_of_candidates(ctx,ctx->current_sequence,1))
    {
        result = true;
        for (int j = 1; (j < ctx->spectrum_size) && result; j++) {
            result = result && ctx->current_psd[j] <= ctx->threshold;
        }
...
void process_and_filter_vectors_dfs(CosetList *cl, int N, int p, int q,
    ...
    int *current_psd = calloc((size_t)(N + 1), sizeof(int));

    if (!current_bound || !combination || !suffix_ones || !current_sequence || !remaining_bits ||
        !is_less_compression_a || !is_less_compression_b || !current_psd) {
